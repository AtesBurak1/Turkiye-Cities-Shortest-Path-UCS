...
std::vector< std::string> Graph::uniform_cost_search(std::string src, std::string dst, int &cost, int *visited) {
    ...
    //Set initial distance to max (for fnid shortest one)
    ...
    while (!pq.empty()) { // loop until queue is empty
        ...
        if (currentCity == dst) { // shortest path found !
        ...
        for (const auto& neighbor : adj[currentCity]) { // visit every neighbor of current city
...
std::vector <std::string> Graph::k_nearest_search(std::string src, int *visited, int count, int k)
{
  ...
  while (!pq.empty()) { // loop until queue is empty
      ...
      if (count == k) { // shortest path found !
